...
Artisan::command('debt:run-schedules', function (ExecutePaymentSchedule $execute): void {
    PaymentSchedule::query()->where('mode', 'automatic')->where('status', 'active')->whereNotNull('next_runs_on')->whereDate('next_runs_on', '<=', today())->each(function (PaymentSchedule $schedule) use ($execute): void {
